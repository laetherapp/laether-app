...
export default function CerclesPage() {
  ...
  const weekQuestion = questions[week % questions.length];
  const weekNum = (week % questions.length) + 1;

  const [status, setStatus] = useState('loading');
  const [email, setEmail] = useState('');
  const [pseudo, setPseudo] = useState('');
  const [joining, setJoining] = useState(false);
  const [circleData, setCircleData] = useState(null as any);
  const [messages, setMessages] = useState([] as any[]);
  const [newMsg, setNewMsg] = useState('');
  const [sending, setSending] = useState(false);
  const [isPremium, setIsPremium] = useState(false);
  const [bubbleId, setBubbleId] = useState(null as string | null);
  ...
  useEffect(() => {
    ...
    if (saved) { checkStatus(saved); }
    else { setStatus('none'); }
  }, []);

  const checkStatus = async (em: string) => {
    setStatus('loading');
    try {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (msg?.id) setMessages((m: any[]) => [...m, msg]);
      setTimeout(() => chatRef.current?.scrollTo({ top: 9999, behavior: 'smooth' }), 100);
    } catch {}
    setSending(false);
  };

  const handleTranslate = async (msgId: string, text: string, from: string) => {
    if (!isPremium) { setBubbleId(msgId); return; }
    const to = from === 'fr' ? 'en' : 'fr';
    setMessages((m: any[]) => m.map(msg => msg.id === msgId ? { ...msg, translating: true } : msg));
    try {
      ...
      const data = await res.json();
      setMessages((m: any[]) => m.map(msg => msg.id === msgId ? { ...msg, translation: data.translation, translating: false } : msg));
    } catch {
      setMessages((m: any[]) => m.map(msg => msg.id === msgId ? { ...msg, translating: false } : msg));
    }
  };

  const s = { color: 'rgba(40,30,15,0.88)' };
  const sm = { color: 'rgba(40,30,15,0.55)' };
  const sg = { color: 'var(--gold)' } as any;
  ...
  return (
    <section className="min-h-screen flex flex-col pt-24 pb-32 px-5">
      <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}
        ...
        <p className="text-[10px] uppercase tracking-[0.22em]" style={{ ...sg, opacity: 0.9 }}>{t('eyebrow')}</p>
        <div className="divider" />
        <div className="space-y-1">
          {t('title').split('\n').map((line: string, i: number) => (
            <h1 key={i} className="font-display text-[30px] sm:text-[36px] leading-[1.2]"
              style={{ fontWeight: 500, ...s }}>{line}</h1>
          ))}
        </div>
      </motion.div>
        ...
        <AnimatePresence mode="wait">
          ...
          {status === 'loading' && (
            <motion.p key="loading" initial={{ opacity: 0 }} animate={{ opacity: 0.5 }} exit={{ opacity: 0 }}
              className="text-center text-[13px]" style={sm}>Un instant…</motion.p>
          )}
            ...
            <motion.div key="none" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="glass-strong rounded-3xl p-7">
                <div className="flex items-center justify-between mb-4">
                  <p className="text-[10px] uppercase tracking-[0.14em]" style={{ ...sg, opacity: 0.9 }}>{t('question_label')}</p>
                  <span className="text-[10px]" style={{ color: 'rgba(40,30,15,0.35)' }}>{t('week')} {weekNum}</span>
                </div>
                <p className="font-display italic text-[17px] leading-[1.75]" style={{ ...s, fontWeight: 400 }}>"{weekQuestion}"</p>
              </div>
              ...
              <div className="glass-strong rounded-3xl p-7 text-center">
                ...
                <p className="font-display italic text-[17px] mb-1" style={{ ...s, fontWeight: 400 }}>{t('no_circle')}</p>
                <p className="text-[13px] mb-5" style={sm}>{t('no_circle_body')}</p>
              ...
              <div className="glass rounded-3xl p-6 space-y-4">
                {(locale === 'fr' ? [
                  ...
                  ['4.', 'Les messages peuvent être traduits FR/EN.'],
                ] : [
                  ['1.', 'You join a circle of 3, formed automatically.'],
                  ['2.', 'Each week, a question guides your exchange.'],
                  ['3.', 'You share at your own pace, in writing, freely.'],
                  ['4.', 'Messages can be translated FR/EN.'],
                ]).map(([num, text], i) => (
                  <div key={i} className="flex gap-4">
                    <span className="font-display italic text-[15px] mt-0.5 shrink-0" style={{ ...sg, fontWeight: 400 }}>{num}</span>
          ...
          {status === 'join_form' && (
            <motion.div key="form" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
              ...
              <p className="font-display italic text-[20px] text-center" style={{ ...s, fontWeight: 400 }}>
                {locale === 'fr' ? 'Rejoindre un cercle' : 'Join a circle'}
              </p>
              <div className="divider" />
              <div className="space-y-2">
                <label className="text-[11px] uppercase tracking-[0.12em]" style={sm}>
                ...
                <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                  ...
                  style={{ border: '1px solid rgba(184,148,74,0.25)', ...s, background: 'rgba(255,255,255,0.3)' }} />
              </div>
              <div className="space-y-2">
                <label className="text-[11px] uppercase tracking-[0.12em]" style={sm}>
              ...
              <button onClick={() => setStatus('none')} className="w-full text-center text-[11px] py-2"
          ...
          {status === 'waiting' && (
            <motion.div key="waiting" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="divider" />
              <p className="font-display italic text-[20px]" style={{ ...s, fontWeight: 400 }}>{t('waiting')}</p>
              <p className="text-[13px] whitespace-pre-line" style={sm}>{t('waiting_body')}</p>
              <p className="text-[11px]" style={{ color: 'rgba(40,30,15,0.35)' }}>
          ...
          {status === 'active' && circleData && (
            <motion.div key="active" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="glass-strong rounded-3xl p-6">
                ...
                <p className="font-display italic text-[16px] leading-[1.7]" style={{ ...s, fontWeight: 400 }}>"{weekQuestion}"</p>
              </div>

              <div className="glass rounded-2xl px-5 py-3 flex items-center gap-3">
                <div className="flex gap-1.5">
                  {circleData.members?.map((m: any, i: number) => (
                ...
                <p className="text-[11px]" style={sm}>{circleData.members?.map((m: any) => m.pseudo).join(' · ')}</p>
              </div>
              ...
              <div ref={chatRef} className="glass-strong rounded-3xl p-5 space-y-4 overflow-y-auto"
                ...
                {messages.map((msg: any) => {
                  const myPseudo = circleData.pseudo || (typeof window !== 'undefined' ? localStorage.getItem('circle_pseudo') : '');
                  const isMe = msg.pseudo === myPseudo;
                  ...
                  return (
                    <motion.div key={msg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                      ...
                      <div className={'rounded-2xl px-4 py-3 max-w-[85%] ' + (isMe ? 'rounded-tr-sm' : 'rounded-tl-sm')}
                        ...
                        <p className="text-[14px] leading-[1.65]" style={s}>{msg.content}</p>
                        {msg.translation && (
                      ...
                      {canTranslate && !msg.translation && (
                        <div className="relative">
                          <button onClick={() => handleTranslate(msg.id, msg.content, msg.language)}
                            ...
                            <span>{msg.translating ? '…' : (locale === 'fr' ? 'Voir en français' : 'See in English')}</span>
                            {!isPremium && <span style={{ fontSize: '9px' }}>🔒</span>}
                          </button>
                          {bubbleId === msg.id && !isPremium && (
                            <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}
                              className="absolute bottom-6 left-0 z-10 rounded-2xl p-3"
                              style={{ background: 'rgba(255,255,255,0.95)', border: '1px solid rgba(184,148,74,0.2)', minWidth: '200px' }}>
                              <p className="text-[11px] mb-2" style={sm}>
                                {locale === 'fr' ? "La traduction est disponible avec l'accès complet." : 'Translation is available with full access.'}
                              </p>
                              <a href={'/' + locale + '/premium'} className="text-[10px] uppercase tracking-[0.12em]" style={sg}>
                                {locale === 'fr' ? 'Découvrir →' : 'Learn more →'}
                              </a>
                              <button onClick={() => setBubbleId(null)} className="block mt-1 text-[10px]" style={{ color: 'rgba(40,30,15,0.3)' }}>✕</button>
                            </motion.div>
                          )}
                        </div>
                      )}
              ...
              <div className="glass-strong rounded-2xl px-4 py-3 flex items-end gap-3">
                <textarea value={newMsg} onChange={e => setNewMsg(e.target.value)}
                  ...
                  className="flex-1 bg-transparent resize-none text-[14px] focus:outline-none leading-relaxed" style={s}
